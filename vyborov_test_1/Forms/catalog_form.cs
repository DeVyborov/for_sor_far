    ...
    public partial class catalog_form : Form
    {
        ...
        private void catalog_form_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT COUNT(id) FROM auth", SQLclass.str);
                ...
                SqlCommand cmd_get_info = new SqlCommand("SELECT * FROM auth", SQLclass.str);
                ...
                {
                    book_info_panel book_Info_Panel = new book_info_panel(logins[i], password[i], Convert.ToInt32(ids[i])) { Dock = DockStyle.Top};
